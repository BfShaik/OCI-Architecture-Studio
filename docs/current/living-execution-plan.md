...
| TASK-054 | Done | Run OCI GenAI synthesis live parity and promote after gates pass. | Staging uses OCI GenAI synthesis with `xai.grok-4.3`; public `/architecture-review` returns `synthesis_provider=oci_genai` with no fallback. GenAI go-live gates passed after the SaaS load-balancing grounding fix. |
| TASK-055 | Done | Migrate embeddings from local hash to OCI GenAI semantic embeddings. | Promoted `cohere.embed-v4.0` at 1536 dimensions after `cohere.embed-english-v3.0` was unavailable in the tenancy/region and the operator approved the best visible model. Rebuilt a 60-chunk OCI GenAI manifest, loaded `OCI_ARCHITECTURE_CHUNKS_V4`, uploaded the v4 manifest to Object Storage, preserved `oci-rag-index.local-hash.json` for rollback, flipped staging runtime, and passed `/retrieval/health`, 32/32 retrieval regression, 18/18 golden evals, and 8/8 edge evals. |

...
| TASK-055 | Gated | Migrate embeddings from local hash to OCI GenAI semantic embeddings. | Operator approved `cohere.embed-v4.0` as the best visible candidate after `cohere.embed-english-v3.0` was not listed in the tenancy/region. Added the D1 mismatch guardrail and migration-readiness code only: retrieval refuses configured embedder/index metadata mismatches, ingestion uses `SEARCH_DOCUMENT`, query retrieval uses `SEARCH_QUERY`, manifests/Oracle row metadata carry embedding provider/model/dimensions, and `/retrieval/health` exposes the verdict. No reindex, Object Storage upload, staging env flip, or provider promotion was performed because `TASK-054` remains blocked and v4 changes the target dimensions to 1536. |

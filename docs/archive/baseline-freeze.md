...
Current-state note: this document freezes the original post-deploy baseline. Staging was later promoted to active `oracle_ai_vector_search`, OCI GenAI synthesis, and OCI GenAI embeddings; see `docs/current/status.md` and `docs/current/living-execution-plan.md` for the current runtime.

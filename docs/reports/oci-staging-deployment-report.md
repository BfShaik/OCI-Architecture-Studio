...
Current-state note: this report records the initial infrastructure apply. Staging was later promoted to active `oracle_ai_vector_search`, OCI GenAI synthesis, and OCI GenAI embeddings with Object Storage/local rollback paths. See `docs/current/status.md` and `docs/current/living-execution-plan.md` for the current runtime.
